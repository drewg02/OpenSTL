...
def save_visualizations(ex_name, pre_seq_length, aft_seq_length):
    ...
    example_idx = 0
    show_video_line(inputs[example_idx], ncols=pre_seq_length, vmax=212, vmin=0, cbar=False, format='png', cmap='Reds',
                    out_path=f'./work_dirs/{ex_name}/saved/2dplate_input.png')
    show_video_line(preds[example_idx], ncols=aft_seq_length, vmax=212, vmin=0, cbar=False, format='png', cmap='Reds',
                    out_path=f'./work_dirs/{ex_name}/saved/2dplate_pred.png')
    show_video_line(trues[example_idx], ncols=aft_seq_length, vmax=212, vmin=0, cbar=False, format='png', cmap='Reds',
                    out_path=f'./work_dirs/{ex_name}/saved/2dplate_true.png')
    
    diff = np.abs(preds[example_idx] - trues[example_idx])
    show_video_line(diff, ncols=aft_seq_length, vmax=212, vmin=0, cbar=False, format='png', cmap=cmap_diff,
                    out_path=f'./work_dirs/{ex_name}/saved/2dplate_diff.png')
    
    show_video_line_tsse(trues[example_idx], preds[example_idx], ncols=aft_seq_length, vmax=212, vmin=0, cbar=False, format='png',
                         cmap='Reds',
                         out_path=f'./work_dirs/{ex_name}/saved/2dplate_tsse.png')
    
    show_video_gif_multiple(inputs[example_idx], trues[example_idx], preds[example_idx], vmax=212, vmin=0, cmap='Reds',
