...
def main():
    ...
    if not config.get('inference') and not config.get('test'):
        config['test'] = True
    ...
    if config.get('inference') and not config.get('test'):
    ...
    for j in range(sample_length - 1):
        eval_res, result_data = func(save_files=False)
        if eval_res:
        ...
        exp.test_loader = create_dataloader(data, args.pre_seq_length, args.aft_seq_length, args.val_batch_size, False,
                                            False, args.dist)
    ...
    if rank == 0:
        ...
        if has_nni and 'mse' in final_eval:
